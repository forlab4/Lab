...
namespace project_vniia
{
    public partial class Form_protokol : Form
    {
        ...
        {
            int tt = 0, tt1 = 0, tt2=0, tt3=0, tt4=0,tt5=0;
            ...
            if (myDBs["[Проверка]"].table.Columns.Contains("Дата проверки"))
            {
                tt5 = myDBs["[Проверка]"].table.Columns["Дата проверки"].Ordinal;
            }
            int kolvo = 0, lol=0;
            DateTime time;
            foreach (var typee in blocks)
            {
                ...
                if (typee != "")
                {
                    time = new DateTime(0001, 1, 1, 0, 0, 0);
                    var newDataTable = myDBs["[Проверка]"].table.AsEnumerable()
                        .OrderBy(r => r.Field<DateTime>("Дата проверки")).CopyToDataTable();
                    foreach (DataRow srt_ in newDataTable.Rows)//(DataRow srt_ in myDBs["[Проверка]"].table.Rows)
                    {
                        if (srt_[tt3].ToString() == typee)
                        {
                            if (srt_[tt5].ToString() != "" && Convert.ToDateTime(srt_[tt5]) > time)
                            {
                                time = Convert.ToDateTime(srt_[tt5]);
                                if (srt_[tt].ToString() != "" && Convert.ToDouble(srt_[tt]) != 0)
                                    znach_SU50[lol, kolvo] = srt_[tt].ToString();
                                if (srt_[tt1].ToString() != "" && Convert.ToDouble(srt_[tt1]) != 0)
                                    znach_SCs50[lol, kolvo] = srt_[tt1].ToString();
                                if (srt_[tt2].ToString() != "" && Convert.ToDouble(srt_[tt2]) != 0)
            ...
            foreach (var typee in blocksN)
            {
                ...
                if (typee == null || typee == "")
                    ...
                    else
                    {
                        var newDataTable = myDBs["[Проверка]"].table.AsEnumerable()
                            .OrderBy(r => r.Field<DateTime>("Дата проверки")).CopyToDataTable();
                        foreach (DataRow srt_ in newDataTable.Rows)
                        //foreach (DataRow srt_ in myDBs["[Проверка]"].table.Rows)
                        {
                            if (srt_[tt3].ToString() == typee)
                            {
                                ...
                                {
                                    time = Convert.ToDateTime(srt_[tt5]);
                                    if (srt_[tt4].ToString() != "" && Convert.ToDouble(srt_[tt4]) != 0)
                                        znach_SCfCm[lol, kolvo] = srt_[tt4].ToString();
                                }
                            }
                        }
